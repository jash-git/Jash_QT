...
void DialogImpl::ClearDB()
{
	...
	DelFile.remove();
	QFile DelFile01("timer_time.log");
	DelFile01.remove();
}
...
void DialogImpl::RunVLCInputBash(int intswitch)
{
	...
	switch(intswitch)
	{
		case 0://VOD
			...
			m_argumentsVOD<<m_StrMP4Path<<StrDiffSec;//2013-10-07
...
void DialogImpl::CalcDiffSec(float Now,float DB)
{
	long lngNow,lngDB;
	long lngSumNowSec,lngSumDBSec;
	long lngDiffSec;
	int hr,min,sec;
	...
	hr=lngNow/10000;
	min=(lngNow-(hr*10000))/100;
	sec=(lngNow-(hr*10000)-(min*100));
	lngSumNowSec+=hr*60*60;//hr -> sec
	lngSumNowSec+=min*60;
	lngSumNowSec+=sec;
	
	hr=lngDB/10000;
	min=(lngDB-(hr*10000))/100;
	sec=(lngDB-(hr*10000)-(min*100));
	lngSumDBSec+=hr*60*60;//hr -> sec
	lngSumDBSec+=min*60;
	lngSumDBSec+=sec;
	
	lngDiffSec=lngSumNowSec-lngSumDBSec;

	FILE *pfTT;
	
	pfTT=fopen("timer_time.log","a");
	fprintf(pfTT,"CalcDiffSec::lngSumNowSec=%ld,lngSumDBSec=%ld,lngDiffSec=%ld\n",lngSumNowSec,lngSumDBSec,lngDiffSec);
	fclose(pfTT);
	
	StrDiffSec.append(QString("%1").arg(lngDiffSec));
...
void DialogImpl::slotTimer()
{
	...
	pfTT=fopen("timer_time.log","a");
	fprintf(pfTT,"init::fltDate=%f,fltTime=%f,fltAfterDate=%f,fltAfterTime=%f\n",fltDate,fltTime,fltAfterDate,fltAfterTime);
	fclose(pfTT);
	
	if(blnDelayCheck==true)
	{
		if( (fltAfterDate>fltDate) || (fltTime < fltAfterTime) )
		{
			pfTT=fopen("timer_time.log","a");
			fprintf(pfTT,"return:fltDate=%f,fltTime=%f,fltAfterDate=%f,fltAfterTime=%f\n",fltDate,fltTime,fltAfterDate,fltAfterTime);
			fclose(pfTT);
			return;
		}
	...
	else
	{
		...
		if(intDBCount==0)
		{
			...
			QApplication::exit();
			pfTT=fopen("timer_time.log","a");
			fprintf(pfTT,"intDBCount=0\n");
			fclose(pfTT);
			ClearDB();
			return;
		}
		pfTT=fopen("timer_time.log","a");
		fprintf(pfTT,"intDBCount=%d\n",intDBCount);
		fclose(pfTT);
		for (int i = 0; i < model->rowCount(); ++i)
		{
    		QSqlRecord record = model->record(i);
			...
			{
				pfTT=fopen("timer_time.log","a");
				fprintf(pfTT,"true:fltDate=%f,fltTime=%f,fltDB_Date=%f,fltDB_Time=%f\n",fltDate,fltTime,fltDB_Date,fltDB_Time);
				fclose(pfTT);
				QSqlQuery rs(db);
				blncheckDate=false;
				blncheckTime=false;
				...
				if(StrWork!="live")
				{
					...
					CalcDiffSec(fltTime,fltDB_Time);
			...
			{
				pfTT=fopen("timer_time.log","a");
				fprintf(pfTT,"false:fltDate=%f,fltTime=%f,fltDB_Date=%f,fltDB_Time=%f\n",fltDate,fltTime,fltDB_Date,fltDB_Time);
				fclose(pfTT);
				fltAfterTime=fltDB_Time;
				fltAfterDate=fltDB_Date;
