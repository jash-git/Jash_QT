...
void DialogImpl::slotTimer()
{
	...
	int intNowHour,intHour;
	int intNowMin,intMin;
	intNowHour=time.hour();
	intNowMin=time.minute();
	QString StrNowDate=date.toString("yyyy-MM-dd");
	QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
	QFile FilecheckDB("JL.db");
	bool blncheckDB=false;
	blncheckDB=FilecheckDB.exists();//判斷檔案是否存在
	...
	else
	{
		...
		for (int i = 0; i < model->rowCount(); ++i)
		{
			...
			intMin=StrMinutes.toInt();
			if((intHour==intNowHour) && (intMin<=intNowMin))
			{
				blncheckTime=true;
			}
			if(StrNowDate==StrDate)
			{
				blncheckDate=true;
			}
			if((intid!=1) && (blncheckDate==true) && (blncheckTime==true))
			{
				...
				QString string = codec->toUnicode(strSql);
				rs.exec(string);
				QMessageBox::warning(0, QObject::tr("You have a Note..."),StrNote);
