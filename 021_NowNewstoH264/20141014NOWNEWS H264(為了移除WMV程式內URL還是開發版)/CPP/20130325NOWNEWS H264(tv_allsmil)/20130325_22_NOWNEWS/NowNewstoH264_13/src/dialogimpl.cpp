...
void DialogImpl::slotTimer1()
{
	...
	switch(m_intStep)
	{
		case 0:
			...
			//StrBuf="http://5tvadmin.tw-airnet.net/cgi-php/media_transform2.php?op=get_nownews&media_type=h264&datetime=";
			StrBuf="http://dev5tvadmin.tw-airnet.net/cgi-php/media_transform2.php?op=get_nownews&media_type=h264&datetime=";
		...
		case 1:
			...
			StrSP=StrListURL.at(m_lngWMVCount);//jash write at 2014/10/09 StrSP=StrListSP.at(m_lngWMVCount);
			StrDP=StrListDP.at(m_lngWMVCount);
			LineEdit04->setText(StrFileName);
			////////////////////////////
			pflog=fopen("nownewsfilename.log","a");
		...
		case 4:
			...
			else
			{
				m_intStep=0;
				UpdateNOCFun();
				//QApplication::quit();//模擬用 at 2010-09-07
			}
			timer1->start(3000);
			break;
	}
...
void DialogImpl::UpdateNOCFun()
{
	...
	StrBuf="http://noc.tw-airnet.net/monitor/wService.aspx?OP=SetServiceStatusByID&ServiceID=327&Status=0&Value=";
...
void DialogImpl::KeepLiveFun()
{
	...
	if(!m_blnkeeplive)
	{
		StrUrl="http://noc.tw-airnet.net/monitor/wService.aspx?OP=KeepaLive&ServiceID=327";
		url1.setUrl(StrUrl);
	}
	else
	{
		StrUrl="http://noc.tw-airnet.net/monitor/wService.aspx?OP=GetServiceStatusByID&ServiceID=327";
		url1.setUrl(StrUrl);		
	}
	WView01->setUrl(url1);
	m_blnkeeplive=!m_blnkeeplive;
}
void DialogImpl::UpdateDBFun(QString StrId,int intfun)
{
	QUrl url1;
	QString StrUrl;
	//StrUrl="http://5tvadmin.tw-airnet.net/cgi-php/media_transform2.php?op=nownews_write2db&media_type=h264&mi_id=";
	if(intfun==0)
	{
		StrUrl="http://dev5tvadmin.tw-airnet.net/cgi-php/media_transform2.php?op=nownews_lock&media_type=h264&mi_id=";
	}
	else if(intfun==1)
	{
		StrUrl="http://dev5tvadmin.tw-airnet.net/cgi-php/media_transform2.php?op=nownews_write2db&media_type=h264&mi_id=";	
	}
	StrUrl+=StrId;
	url1.setUrl(StrUrl);
	WView01->setUrl(url1);
...
void DialogImpl::slotReadNodeList()
{
	...
	StrListSP.clear();//jash write at 2014/10/09
	StrListDP.clear();//jash write at 2014/10/09
	StrListURL.clear();//jash write at 2014/10/09
	...
	while(!docNoderoot.isNull())
	{
		...
		for(k=0;k<l;k++)
		{
			...
			if(!docElement.isNull())
			{
				...
				if(k==6)//jash write at 2014/10/09
				{//jash write at 2014/10/09
					StrData=StrData.mid(0,4)+StrData.mid(5,2)+StrData.mid(8,2);//jash write at 2014/10/09
					StrData="ftp://jash:chwbnnoc@61.67.114.50/01NEWS/"+StrData;//jash write at 2014/10/09
					StrData=StrData+"/";//jash write at 2014/10/09
					StrListURL << StrData;//jash write at 2014/10/09
				}//jash write at 2014/10/09
				if(k==7)
				{
					//StrListSP,StrListDP
