...
void consoletimer::Xml2Txt()
{
	...
	QString StrPath;
	QString StrData;
	QByteArray ArrlistFile,Arrplaylist;
	QByteArray Arrsortlist;
	...
	srand(time(NULL));
	m_intrand=(rand()%3);
	snprintf(chrbuf, sizeof(chrbuf), "%d", m_intrand);
	
	m_StrMainFileName=StrPath.mid((StrPath.length()-3),2);
	m_StrMainFileName=m_StrMainFileName+"_";
	m_StrMainFileName=m_StrMainFileName+chrbuf;
	
	listFile="/tmp/list_";
	listFile+=m_StrMainFileName;
	playlist="/tmp/playlist_";
	playlist+=m_StrMainFileName;
	playlist+=".txt";
	
	sortlist="sort --random-sort "+listFile;
	for(int i=0;i<m_intrand;i++)
	{
		sortlist+=";";
		sortlist+="sort --random-sort "+listFile;
	}
	
	ArrlistFile=listFile.toAscii();
	Arrplaylist=playlist.toAscii();
	Arrsortlist=sortlist.toAscii();
	QByteArray byteArray=m_StrMp4Path.toLocal8Bit ();
	
	sprintf(cmd,"find %s -name \"*.mp4\" > %s;%s >%s",byteArray.data(),ArrlistFile.data(),Arrsortlist.data(),Arrplaylist.data());
	//cout<<"cmd＝"<<cmd<<endl;
...
void consoletimer::slotTimer()
{
	...
	QByteArray ArrPath,ArrMainFileName;
	...
	ArrPath=StrPath.toAscii();
	ArrMainFileName=m_StrMainFileName.toAscii();
	...
	cout<<"目前路徑＝"<<ArrPath.data()<<endl;
	cout<<"ArrMainFileName＝"<<ArrMainFileName.data()<<endl;
	cout<<endl;
	cout<<"-------------------------------------------------"<<endl;
