...
int main(int argc, char *argv[])
{
	...
	if(file != cgi.getFiles().end())
	{
		...
		*/
		...
		FILE *pfList;
		pfList=fopen("uploadlist.log","a");
		fprintf(pfList,"%d-%d-%d %d:%d:%d,",(now->tm_year + 1900),(now->tm_mon + 1),now->tm_mday,now->tm_hour,now->tm_min,now->tm_sec);
		fprintf(pfList,"/cgi-bin/Upload_File_Folder/%s\n",strName.c_str());		
		fclose(pfList);	
	}
	printf("</body>\r\n\r\n");
	printf("</HTML>\r\n\r\n");	
